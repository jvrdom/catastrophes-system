...
var app = {
    ...
    onDeviceReady: function() {
        console.log('lgh en onDeviceReady ...' );
    ...
    openDb: function() {
		...
		console.log('lgh en openDb window.navigator.simulator === true ');
            // For debugin in simulator fallback to native SQL Lite
            console.log("Use built in SQL Lite");
			app.db = window.localStorage.getItem("appDb");
			console.log(app.db);
            //app.db = window.openDatabase("Todo", "1.0", "Cordova Demo", 200000);

        }
        else {
		console.log('no uso el built in SQL Lite, uso localstorage para modulo rescatista web');
		// traigo la base de localstorage
		app.db = window.localStorage.getItem("appDb");
		//console.log(app.db);
    ...
    createTable: function() {
		...
		// }
		//var db.tabla = null;
		//console.log('en createTable tabla='+tabla);
		console.log('en createTable app.db='+app.db);
		
		
		//app.db.tabla = null;
		if (app.db.tabla == null) console.log('app.db.tabla es null');
		console.log("app.db.catastrfe: "+app.db.catastrofe);
		
		app.db['catastrofe']='jsonCatastrofe';
		console.log("update app.db.catastrfe: "+app.db.catastrofe);
...
//idC, nombre, latitud, longitud, planRiesgo, planEmergencia
		...
		*/
		...
		//data = app.db.catastrofe;
		var data = window.localStorage.getItem("appDbCatastrofe");
		console.log('data '+ data);
		
		$.each(JSON.parse(data), function(idx, obj) {
			// si mando un alert no me manda el leaflet, wtf
			//alert(obj.nombre +' '+ obj.id);
			if(obj.id == id){
				app.nombre = obj.nombre;
				app.planRiesgo = obj.planRiesgo;
				app.planEmergencia = obj.planEmergencia;
				app.latitud = obj.latitud;
				app.longitud= obj.longitud;
				var hrefRiesgo =  app.planRiesgo;
				var hrefEmergencia =  app.planEmergencia;
				$("#planriesgo").attr("href", hrefRiesgo);
				$("#planemergencia").attr("href", hrefEmergencia);
...
// la otra forma de iterar, hay que parsear el json para que no de error en consola chrome
...
*/
    ...
    refreshCatastrofe: function() {
		...
		//var dataCatastrofes = app.db.catastrofe;
        //var render = function (tx, rs) {
            //alert('en render');
...
//            }
        	...
        	var listaCatastrofes = window.localStorage.getItem("appDbCatastrofe");
        	var rowOutput ="";
        	//var rowOutput ={};
        	
        	//var salida = function(){
			$.each(JSON.parse(listaCatastrofes), function(idx, obj) {
				idCatastrofe = obj.id;
				rowOutput += renderCatastrofe(obj);
				console.log('lgh en resfreshCatastrofe rowOutput '+ rowOutput);
...
/* CAMBIO: en vez de recorrer la coleccion y hacer los inserts asigno directamente la coleccion de catastrofes
en el objeto app.db.catastrofe y localstorage
                $.each(data, function(i, obj) {
                  //use obj.id and obj.name here, for example:
                  console.log('lgh llamando addCatastrofe para '+obj.nombre);
                  app.addCatastrofe(obj.idC, obj.nombre, obj.latitud, obj.longitud, obj.planRiesgo, obj.planEmergencia );
				...
				*/
				// ya hice opendb antes, ahora actualizo las catastrofes en la app y localstorage
                console.log('catastrofes data: '+ JSON.stringify(data));
				datajson = JSON.stringify(data);
				var idCatastrofe = null;
				$.each(JSON.parse(datajson), function(idx, obj) {
					idCatastrofe = obj.id;
					return false;
				});
				
				app.db['catastrofe']= datajson;
				
				window.localStorage.setItem("appDbCatastrofe", datajson);
                console.log('lgh en success de getCatastrofes ');
                //setCatastrofeYRefresh();
                var seleccionada = window.localStorage.getItem("appCatastrofe");
                console.log('lgh antes de app.setAppCatastrofe(1); y app.refreshCatastrofe(); ');
                //app.setAppCatastrofe(seleccionada); 
                app.setAppCatastrofe(idCatastrofe); 
                app.refreshCatastrofe();
            },
            error: function(xhr, textStatus, errorThrown) {
                //setCatastrofeYRefresh();
...
//app.hostservidor = "127.0.0.1";
app.hostservidor = "localhost";
